...
const createDeliveryOptions = (parentNode, input) =>
  forNodeEach(
    parentNode.node("delivery-options"),
    input,
    (deliveryOptions, entry) => {
      ...
      if (Array.isArray(entry.days))
        option.att("days", `${entry.days[0]}-${entry.days[1]}`);
    }
  );

const createPickupOptions = (parentNode, input) =>
  forNodeEach(
    parentNode.node("pickup-options"),
    input,
    (pickupOptions, entry) => {
      const option = pickupOptions.node("option", cleanse(entry));
...
const createConditionOptions = (parentNode, input) => {
  // console.log(input);
  const condition = parentNode.node("condition");
  condition.node("reason").txt(input.reason);
  condition.node("quality").txt(input.quality);

  condition.att("type", input.type);
  ...
  forNode(parentNode.node("offer"), input, (offer, key, entry) => {
    ...
    if (typeof entry !== "undefined" && entry !== null) {
      switch (key) {
        ...
        case "pickup-options":
          createPickupOptions(offer, entry);
          break;

        case "condition":
          createConditionOptions(offer, entry);
          break;
